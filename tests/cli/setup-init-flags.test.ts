...
describe("flaker init --adapter --runner", () => {
  let dir: string;
  ...
  it("writes [adapter] and [runner] sections when flags are provided", () => {
    execSync(`node ${cliPath} init --adapter vitest --runner vitest`, { cwd: dir, stdio: "pipe" });
  ...
  it("writes playwright adapter with correct section when --adapter playwright", () => {
    execSync(`node ${cliPath} init --adapter playwright`, { cwd: dir, stdio: "pipe" });
    const toml = readFileSync(join(dir, "flaker.toml"), "utf-8");
    expect(toml).toMatch(/\[adapter\][\s\S]*type = "playwright"/);
  });

  it("writes runner command matching the runner type", () => {
    execSync(`node ${cliPath} init --runner jest`, { cwd: dir, stdio: "pipe" });
    const toml = readFileSync(join(dir, "flaker.toml"), "utf-8");
  ...
  it("produces default sections when no flags provided", () => {
    execSync(`node ${cliPath} init`, { cwd: dir, stdio: "pipe" });
    const toml = readFileSync(join(dir, "flaker.toml"), "utf-8");
  ...
  it("rejects unknown adapter values", () => {
    expect(() =>
      execSync(`node ${cliPath} init --adapter frobnitz`, { cwd: dir, stdio: "pipe" })
    ).toThrow();
  });

  it("rejects unknown runner values", () => {
    expect(() =>
      execSync(`node ${cliPath} init --runner frobnitz`, { cwd: dir, stdio: "pipe" })
    ).toThrow();
  });
});
